...
contract Avtars is Ownable, ERC721Enumerable {
    ...
    event directReferralRewardTransferred(address user, address directReferrer, uint rewardAmount);
    event indirectReferralRewardTransferred(address user, address indirectReferrer, uint rewardAmount);
    ...
    function mintNFT1(uint _level, uint _mintPrice, bool _delegate, address _referrer) public {
        uint level = _level - 1;
        require(level >= 0 && level <= 7, "Invalid NFT level");
        require(!hasTokens[level][msg.sender], "You already have an NFT of this level!");
        require(!paused, "Minting is paused");
        ...
        if (msg.sender != owner() && !whitelisted[msg.sender]) {
            ...
            uint directReferralReward = requiredPrice / 10;
            uint indirectReferralReward = requiredPrice * 5 / 100;
            uint requiredPriceAfterRef = requiredPrice;
            if (directReferrer != address(0)) {
                requiredPriceAfterRef -= directReferralReward;
            }
            if (indirectReferrer != address(0)) {
                requiredPriceAfterRef -= indirectReferralReward;
            }
            require(_mintPrice >= requiredPriceAfterRef, "Insufficient payment amount");
            if (_delegate) {
                uint sharePrice = requiredPrice * 10 / 100;
                uint newMintPrice = requiredPrice + sharePrice;
                require(_mintPrice >= newMintPrice, "Insufficient payment amount; if delegate is true, add 10% more.");
                uint shareToDelegate = _mintPrice - requiredPrice;
                if (indirectReferrer != address(0)) {
                    IERC20(_token).safeTransferFrom(msg.sender, indirectReferrer, indirectReferralReward);
                    IERC20(_token).safeTransferFrom(msg.sender, directReferrer, directReferralReward);
                    IERC20(_token).safeTransferFrom(msg.sender, address(this), requiredPriceAfterRef);
                    IERC20(_token).safeTransferFrom(msg.sender, delegateAddress, shareToDelegate);
                ...
                if (indirectReferrer != address(0)) {
                    uint mintPriceAfterRef = _mintPrice - directReferralReward - indirectReferralReward;
                    IERC20(_token).safeTransferFrom(msg.sender, indirectReferrer, indirectReferralReward);
                    IERC20(_token).safeTransferFrom(msg.sender, directReferrer, directReferralReward);
                    IERC20(_token).safeTransferFrom(msg.sender, address(this), mintPriceAfterRef);
                    emit directReferralRewardTransferred(msg.sender, directReferrer, directReferralReward);
                    emit indirectReferralRewardTransferred(msg.sender, indirectReferrer, indirectReferralReward);
                } else if (directReferrer != address(0)) {
                    uint mintPriceAfterRef = _mintPrice - directReferralReward;
                    IERC20(_token).safeTransferFrom(msg.sender, directReferrer, directReferralReward);
                    IERC20(_token).safeTransferFrom(msg.sender, address(this), mintPriceAfterRef);
                    emit directReferralRewardTransferred(msg.sender, directReferrer, directReferralReward);
                } else {
    ...
    //work for transfering to delegate, direct and indirect , from contract to address 
    function mintNFT3(uint _level, uint _mintPrice, bool _delegate, address _referrer) public {  //can use this after testing
        uint level = _level - 1;
        require(level >= 0 && level <= 7, "Invalid NFT level");
        require(!hasTokens[level][msg.sender], "You already have an NFT of this level!");
        require(!paused, "Minting is paused");
    ...
    //View
    function checkRank(address _user, uint nftlevel, uint amount) public view returns(bool) {  //can use this after testing
            ...
            if (setUserRank[member]._rankChanged == true && setUserRank[member]._rank > nftlevel) {
                ...
                if (getTeamSaleVolume(member) >= testcost[amount]) {
                    if (amount == 0) {
                        ...
                        if (checkRank(member, newNftlevel, newamount)) {
                            memberrankCount++;
                            if (setUserRank[member]._rankChanged == true && setUserRank[member]._rank <= nftlevel) {
                                memberrankCount--;
                            }
    ...
    function checkRank_opt(address _user, uint nftlevel, uint amount) public view returns(bool) {
        ...
        for (uint i = 0; i < _referrals.length; i++) {
            ...
            if (setUserRank[member]._rankChanged == true && setUserRank[member]._rank > nftlevel) {
                memberrankCount++;
            } else if (hasTokens[nftlevel][member] && memberTeamSale >= targetCost) {
                memberrankCount++;
                if (setUserRank[member]._rankChanged == true && setUserRank[member]._rank <= nftlevel) {
                    memberrankCount--;
                }
    ...
    function getRankupLifting2(address _user) public view returns(uint _rank) {  //can use this after testing
        if (setUserRank[_user]._rankChanged) {
            return setUserRank[_user]._rank;
        }
        uint teamsale = getTeamSaleVolume(_user);
    ...
    function legSearch(address member, uint nftlevel, uint amount) internal view returns(bool) {  //can use this after testing
        if (referrals[member].length == 0) {
            return false;
        }
    ...
    function legSearch_opt(address member, uint nftlevel, uint amount) internal view returns(bool) {
...
1x nft level4
0x7c2d464F82C38D98448744018A4b697A25A082c7,["0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2","0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db","0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB"]
...
{745 reffer}                                     3x investment 100 eth  
...
{15A reffer}                                     3x investment 100 eth  
